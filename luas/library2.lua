...
local GameName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name
                if (tick() - Tick) > 0.05 then
                    watermark_title.Text = string.format("Sipare | Ping = %s, Fps = %u, | "..os.date().." | "..GameName, library.shared.ping, library.shared.fps)
                    window.watermark:UpdateSize()
                    Tick = tick()
                end
            end)
        ...
        function window:NotificationList(info)
            ...
            function window.notificationlist:AddNotification(info)
                ...
                --
                utility:LoadImage(notify__gradient, "gradient", images.gradient)
                --
